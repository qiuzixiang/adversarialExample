...
totalSize= (len(os.listdir(FLAGS.input_dir) ))
  ...
  with tf.Graph().as_default():
    ...
    with tf.train.MonitoredSession(session_creator=session_creator) as sess:
      ...
      for filenames, images in load_images(FLAGS.input_dir, batch_shape):
        ...
        outputx=sess.run(end_points['Predictions'], feed_dict={x_input: images})
        #print(filenames)
        #print(np.argmax(outputx, axis=1))
        idx=idx+1
        if(idx*FLAGS.batch_size<totalSize):
          sizex=np.shape(outputx)[0]

        else:
          sizex=idx*FLAGS.batch_size-totalSize
        for i in range(sizex):
            outputx[i][predicted_classes[i]]=0

        newTarget=np.argmax(outputx, axis=1)
        
        all_images_target_class.update({filenames[i]: np.argmax(outputx, axis=1)[i]
                          for i in range(sizex)})
